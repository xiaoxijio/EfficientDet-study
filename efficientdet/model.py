    ...
    def __init__(self, in_channels, num_anchors, num_layers, pyramid_levels=5, onnx_export=False):
        ...
        self.bn_list = nn.ModuleList(
            [nn.ModuleList([nn.BatchNorm2d(in_channels, momentum=0.01, eps=1e-3) for i in range(num_layers)]) for j in
             range(pyramid_levels)])  # 跟下面Classifier不同的点就是这里head输出的是 锚框数*4(dy, dx, dh, dw)
...
class Classifier(nn.Module):
    ...
    def __init__(self, in_channels, num_anchors, num_classes, num_layers, pyramid_levels=5, onnx_export=False):
        ...
        self.bn_list = nn.ModuleList(
            [nn.ModuleList([nn.BatchNorm2d(in_channels, momentum=0.01, eps=1e-3) for i in range(num_layers)]) for j in
             range(pyramid_levels)])  # 跟上面Regressor不同的点就是这里head输出的是 锚框数*类别
    ...
    def forward(self, inputs):
        ...
        feats = torch.cat(feats, dim=1)
        feats = feats.sigmoid()  # 类别概率 所以还需要激活一下
...
class EfficientNet(nn.Module):
    ...
    def forward(self, x):
        x = self.model._conv_stem(x)  # 就是一个3*3的卷积
        x = self.model._bn0(x)
