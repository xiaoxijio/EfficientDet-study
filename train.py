...
def get_args():
    ...
    parser.add_argument('-p', '--project', type=str, default='data', help='project file that contains parameters')
...
class ModelWithLoss(nn.Module):
    def __init__(self, model, debug=False):
        super().__init__()
        self.criterion = FocalLoss()  # 用于处理类别不平衡问题的损失函数 尤其是处理负样本占比过大的情况
        self.model = model
        self.debug = debug
...
def train(opt):
    params = Params(f'data/{opt.project}.yml')
