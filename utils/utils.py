...
def invert_affine(metas: Union[float, list, tuple], preds):
    ...
    for i in range(len(preds)):
        if len(preds[i]['rois']) == 0:  # 如果当前图像没有预测到任何边界框
            continue
        else:
            if metas is float:  # 缩放比例
...
def preprocess(image_path, max_size=512, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    # 如果使用单独图片，上面的image_path改为*image_path
...
def postprocess(x, anchors, regression, classification, regressBoxes, clipBoxes, threshold, iou_threshold):
    ...
    :return:
    """
    transformed_anchors = regressBoxes(anchors, regression)  # 实际的边界框[xmin, ymin, xmax, ymax]
    transformed_anchors = clipBoxes(transformed_anchors, x)  # 裁剪至原图大小
    scores = torch.max(classification, dim=2, keepdim=True)[0]  # 对每个锚框，获取所有类别中最高的分类得分
    scores_over_thresh = (scores > threshold)[:, :, 0]  # 筛选出得分大于阈值的边界框
    out = []
    for i in range(x.shape[0]):
        ...
        if anchors_nms_idx.shape[0] != 0:
            ...
            out.append({
                'rois': boxes_.cpu().numpy(),  # 边界框坐标
                'class_ids': classes_.cpu().numpy(),  # 类别
                'scores': scores_.cpu().numpy(),  # 置信度分数
            })
        else:
            out.append({
                'rois': np.array(()),
